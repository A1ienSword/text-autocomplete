...
class DistilGPT2:    
    ...
    def _convert_tokens_to_text(self, tokens: List[str]) -> str:
        """Конвертирует список токенов обратно в текст"""
    ...
    def evaluate_rouge(
        self,
        val_texts: List[Union[str, List[str]]],
        ...
        for i, text in enumerate(tqdm(val_texts[:max_samples], desc="Evaluating")):
            ...
            # Конвертируем токены в текст, если необходимо
            if isinstance(text, list):
                text = self._convert_tokens_to_text(text)
